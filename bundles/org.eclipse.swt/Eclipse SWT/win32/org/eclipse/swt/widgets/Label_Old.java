 ...
 */
public class Label_Old extends Control {
	String text = "";
	Image image;
	...
	static {
		...
		DPIZoomChangeRegistry.registerHandler(Label_Old::handleDPIChange,
				Label_Old.class);
	}
	 ...
	 * @see Widget#getStyle
	 */
	public Label_Old(Composite parent, int style) {
	...
	long callWindowProc(long hwnd, int msg, long wParam, long lParam) {
		if (handle == 0)
			return 0;
		/*
		 * Feature in Windows 7. When the user double clicks on the label, the
		 * text of the label is copied to the clipboard. This is unwanted. The
		 * fix is to avoid calling the label window proc.
		 */
		switch (msg) {
	...
	Point computeSizeInPixels(int wHint, int hHint, boolean changed) {
		...
		if (isImageMode) {
			Rectangle rect = DPIUtil.scaleBounds(image.getBounds(),
					this.getZoom(), 100);
			width += rect.width;
			height += rect.height;
		} else {
			...
			if (length == 0) {
				...
				RECT rect = new RECT();
				int flags = OS.DT_CALCRECT | OS.DT_EDITCONTROL
						| OS.DT_EXPANDTABS;
	...
	boolean isUseWsBorder() {
		return super.isUseWsBorder()
				|| ((display != null) && display.useWsBorderLabel);
	}
	...
	public void setEnabled(boolean enabled) {
		 ...
		 * current style. Therefore, style needs to be changed before
		 * #setEnabled(). Note that adding redraw() after #setEnabled() is a
		 * worse solution because it still causes brief old style painting in
		 * #setEnabled().
		 */
	...
	public void setText(String string) {
		...
		updateStyleBits(getEnabled());
		/*
		 * Feature in Windows. For some reason, SetWindowText() for static
		 * controls redraws the control, even when the text has has not changed.
		 * The fix is to check for this case and do nothing.
		 */
	...
	void updateStyleBits(boolean isEnabled) {
		...
		if (!useOwnerDraw && (display.disabledLabelForegroundPixel != -1)
				&& !isEnabled)
			useOwnerDraw = true;
	...
	LRESULT WM_SIZE(long wParam, long lParam) {
		...
		/*
		 * Bug in Windows. For some reason, a label with style SS_LEFT,
		 * SS_CENTER or SS_RIGHT does not redraw the text in the new position
		 * when resized. Note that SS_LEFTNOWORDWRAP does not have the problem.
	...
	LRESULT WM_UPDATEUISTATE(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		/*
		 * Feature in Windows. When WM_UPDATEUISTATE is sent to a static
		 * control, it sends WM_CTLCOLORSTATIC to get the foreground and
		 * background. If any drawing happens in WM_CTLCOLORSTATIC, it
		 * overwrites the contents of the control. The fix is draw the static
		 * without drawing the background and avoid the static window proc.
		 */
		...
		if (redraw) {
			...
			long code = OS.DefWindowProc(handle, OS.WM_UPDATEUISTATE, wParam,
					lParam);
	...
	void wmDrawChildSeparator(DRAWITEMSTRUCT struct) {
		...
		RECT rect = new RECT();
		int lineWidth = getSystemMetrics(OS.SM_CXBORDER);
		int flags = (style & SWT.SHADOW_IN) != 0
				? OS.EDGE_SUNKEN
				: OS.EDGE_ETCHED;
		if ((style & SWT.HORIZONTAL) != 0) {
			int bottom = struct.top
					+ Math.max(lineWidth * 2, (struct.bottom - struct.top) / 2);
			OS.SetRect(rect, struct.left, struct.top, struct.right, bottom);
			OS.DrawEdge(struct.hDC, rect, flags, OS.BF_BOTTOM);
		} else {
			int right = struct.left
					+ Math.max(lineWidth * 2, (struct.right - struct.left) / 2);
	...
	void wmDrawChildImage(DRAWITEMSTRUCT struct) {
		...
		Image image = getEnabled()
				? this.image
				: new Image(display, this.image, SWT.IMAGE_DISABLE);
		gc.drawImage(image, DPIUtil.scaleDown(x, zoom), DPIUtil
				.scaleDown(Math.max(0, (height - imageRect.height) / 2), zoom));
		if (image != this.image)
			image.dispose();
		gc.dispose();
	}
