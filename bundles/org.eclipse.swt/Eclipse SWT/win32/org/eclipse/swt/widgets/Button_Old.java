 ...
 */
public class Button_Old extends Control {
	String text = "", message = "";
	...
	static {
		...
		DPIZoomChangeRegistry.registerHandler(Button_Old::handleDPIChange,
				Button_Old.class);
	}
	 ...
	 * @see Widget#getStyle
	 */
	public Button_Old(Composite parent, int style) {
	...
	void _setImage(Image image) {
		...
		imageList = null;
		if (image != null) {
			imageList = new ImageList(style & SWT.RIGHT_TO_LEFT, 0);
		...
		/*
		 * Bug in Windows. Under certain cirumstances yet to be isolated,
		 * BCM_SETIMAGELIST does not redraw the control when a new image is set.
	...
	void _setText(String text) {
		...
		if (imageList != null) {
			BUTTON_IMAGELIST buttonImageList = new BUTTON_IMAGELIST();
			buttonImageList.himl = imageList.getHandle(0);
			if (text.length() == 0) {
				if ((style & SWT.LEFT) != 0)
					buttonImageList.uAlign = OS.BUTTON_IMAGELIST_ALIGN_LEFT;
				if ((style & SWT.CENTER) != 0)
		...
		if (image != null && text.length() != 0) {
			...
			if ((style & SWT.LEFT) != 0) {
				...
				margin = Math.max(MARGIN,
						(rect.right - rect.left - margin - MARGIN));
			} else {
				margin = Math.max(MARGIN,
	...
	void createHandle() {
		/*
		 * Feature in Windows. When a button is created, it clears the UI state
		 * for all controls in the shell by sending WM_CHANGEUISTATE with
		 * UIS_SET, UISF_HIDEACCEL and UISF_HIDEFOCUS to the parent. This is
		 * undocumented and unexpected. The fix is to ignore the
		 * WM_CHANGEUISTATE, when sent from CreateWindowEx().
		 */
		...
		if (OS.IsAppThemed()) {
			 ...
			 *
			 * NOTE: On Vista this causes problems when the tab key is pressed
			 * for push buttons so disable the theme background drawing for
			 * these widgets for now.
			 */
			if ((style & (SWT.PUSH | SWT.TOGGLE)) == 0) {
			...
			/*
			 * Bug in Windows. For some reason, the HBRUSH that is returned from
			 * WM_CTRLCOLOR is misaligned when the button uses it to draw. If
			 * the brush is a solid color, this does not matter. However, if the
			 * brush contains an image, the image is misaligned. The fix is to
			 * draw the background in WM_CTRLCOLOR.
			 *
			 * NOTE: For comctl32.dll 6.0 with themes disabled, drawing in
			 * WM_ERASEBKGND will draw on top of the text of the control.
			 */
	...
	private boolean customForegroundDrawing() {
		return foreground != -1 && !text.isEmpty()
	...
	void updateImageList() {
		if (imageList != null) {
			...
			/*
			 * Bug in Windows. Under certain cirumstances yet to be isolated,
			 * BCM_SETIMAGELIST does not redraw the control when an image is
			 * set. The fix is to force a redraw.
			 */
			OS.InvalidateRect(handle, null, true);
		}
	}
	...
	void updateSelection(int flags) {
		if (flags != OS.SendMessage(handle, OS.BM_GETCHECK, 0, 0)) {
			/*
			 * Feature in Windows. When BM_SETCHECK is used to set the checked
			 * state of a radio or check button, it sets the WS_TABSTOP style.
			 * This is undocumented and unwanted. The fix is to save and restore
			 * the window style bits.
			 */
			int bits = OS.GetWindowLong(handle, OS.GWL_STYLE);
			if ((style & SWT.CHECK) != 0) {
				if (flags == OS.BST_INDETERMINATE) {
	...
	LRESULT wmColorChild(long wParam, long lParam) {
		if (isRadioOrCheck()) {
			...
			// Issue 848: Control.wmColorChild() contains a hack that paints
			// entire
			...
			// * alt key is pressed
			// The workaround is to temporarily disable parent's
			// 'THEME_BACKGROUND'
	...
	LRESULT WM_GETOBJECT(long wParam, long lParam) {
		/*
		 * Ensure that there is an accessible object created for this control
		 * because support for radio button position in group accessibility is
	...
	LRESULT WM_SETFOCUS(long wParam, long lParam) {
		/*
		 * Feature in Windows. When Windows sets focus to a radio button, it
		 * sets the WS_TABSTOP style. This is undocumented and unwanted. The fix
		 * is to save and restore the window style bits.
		 */
		int bits = 0;
		if ((style & SWT.RADIO) != 0) {
	...
	LRESULT WM_UPDATEUISTATE(long wParam, long lParam) {
		...
		if (result != null)
			return result;
		/*
		 * Feature in Windows. When WM_UPDATEUISTATE is sent to a button, it
		 * sends WM_CTLCOLORBTN to get the foreground and background. If drawing
		 * happens in WM_CTLCOLORBTN, it will overwrite the contents of the
		 * control. The fix is draw the button without drawing the background
		 * and avoid the button window proc.
		 *
		 * NOTE: This only happens for radio, check and toggle buttons.
		 */
		if ((style & (SWT.RADIO | SWT.CHECK | SWT.TOGGLE)) != 0) {
			...
			if (redraw) {
				...
				long code = OS.DefWindowProc(handle, OS.WM_UPDATEUISTATE,
						wParam, lParam);
				return new LRESULT(code);
			}
		}
		/*
		 * Feature in Windows. Push and toggle buttons draw directly in
		 * WM_UPDATEUISTATE rather than damaging and drawing later in WM_PAINT.
		 * This means that clients who hook WM_PAINT expecting to get all the
		 * drawing will not. The fix is to redraw the control when paint events
		 * are hooked.
		 */
		if ((style & (SWT.PUSH | SWT.TOGGLE)) != 0) {
	...
	LRESULT wmNotifyChild(NMHDR hdr, long wParam, long lParam) {
		switch (hdr.code) {
			case OS.NM_CUSTOMDRAW :
				// this message will not appear for owner-draw buttons
				// (currently the ARROW button).
				...
				switch (nmcd.dwDrawStage) {
					case OS.CDDS_PREPAINT : {
						// buttons are ignoring SetBkColor, SetBkMode and
						// SetTextColor
						if (customBackgroundDrawing()) {
							...
							if (useDarkModeExplorerTheme
									&& (OS.WIN32_BUILD >= OS.WIN32_BUILD_WIN11_21H2)) {
								// On Win11, Light theme and Dark theme images
							...
							if (OS.WIN32_BUILD >= OS.WIN32_BUILD_WIN11_21H2) {
								// 'RoundRect' has left/top pixel reserved for
								// border
								l += 1;
								t += 1;
								...
								OS.SelectObject(nmcd.hdc,
										OS.GetStockObject(OS.NULL_PEN));
								OS.RoundRect(nmcd.hdc, l, t, r, b, radius,
										radius);
						...
						if (customForegroundDrawing()) {
							int radioOrCheckTextPadding = getCheckboxTextOffset(
									nmcd.hdc);
							...
							if (image != null) {
								...
								left += (imageWidth + (isRadioOrCheck()
										? 2 * MARGIN
										: MARGIN)); // for SWT.RIGHT_TO_LEFT
													// right and left are
													// inverted

								int x = margin + (isRadioOrCheck()
										? radioOrCheckTextPadding
										: 3);
								int y = Math.max(0,
										(nmcd.bottom - imageBounds.height) / 2);
								gc.drawImage(image, DPIUtil.autoScaleDown(x),
										DPIUtil.autoScaleDown(y));
								gc.dispose();
							}
							...
							OS.SetRect(textRect, left, nmcd.top + border, right,
									nmcd.bottom - border);
							...
							if ((style & SWT.WRAP) != 0) {
								...
								flags |= OS.DT_SINGLELINE; // TODO: this always
															// draws the prefix
							}
							OS.DrawText(nmcd.hdc, buffer, buffer.length,
									textRect, flags | OS.DT_CALCRECT);
							OS.OffsetRect(textRect, 0, Math.max(0,
									(nmcd.bottom - textRect.bottom - border)
											/ 2));
							if (image != null) {
								// The default button with an image doesn't
								// respect the text alignment. So we do the same
								// for styled buttons.
								flags |= OS.DT_LEFT;
								if (!isRadioOrCheck()) {
									OS.OffsetRect(textRect, Math.max(MARGIN,
											(right - textRect.right) / 2 + 1),
											0);
								}
								...
								flags |= OS.DT_RIGHT;
								OS.OffsetRect(textRect, right - textRect.right,
										0);
							} else {
								flags |= OS.DT_CENTER;
								OS.OffsetRect(textRect,
										(right - textRect.right) / 2, 0);
							}
							...
							if ((nmcd.uItemState & OS.CDIS_FOCUS) != 0) {
								...
								if (isRadioOrCheck()) {
									if (text.length() > 0) {
										OS.SetRect(focusRect, textRect.left - 1,
												textRect.top,
												Math.min(nmcd.right,
														textRect.right + 1),
												Math.min(nmcd.bottom,
														textRect.bottom + 1));
									} else {
										/*
										 * With custom foreground, draw focus
										 * rectangle for CheckBox and Radio
										 * buttons considering the native text
										 * padding value(which is DPI aware).
										 * See bug 508141 for details.
										 */
										OS.SetRect(focusRect, nmcd.left + 1
												+ radioOrCheckTextPadding,
												nmcd.top, nmcd.right - 2,
												nmcd.bottom - 1);
									}
								} else {
									OS.SetRect(focusRect, nmcd.left + 4,
											nmcd.top + 4, nmcd.right - 4,
											nmcd.bottom - 4);
								}
	...
	static int getThemeStateId(int style, boolean pressed, boolean enabled) {
		...
		if (hot) {
			switch (direction) {
				case SWT.UP :
					return OS.ABS_UPHOT;
				case SWT.DOWN :
					return OS.ABS_DOWNHOT;
				case SWT.LEFT :
					return OS.ABS_LEFTHOT;
				case SWT.RIGHT :
					return OS.ABS_RIGHTHOT;
			}
		}
		...
		if (pressed) {
			switch (direction) {
				case SWT.UP :
					return OS.ABS_UPPRESSED;
				case SWT.DOWN :
					return OS.ABS_DOWNPRESSED;
				case SWT.LEFT :
					return OS.ABS_LEFTPRESSED;
				case SWT.RIGHT :
					return OS.ABS_RIGHTPRESSED;
			}
		}
		...
		if (!enabled) {
			switch (direction) {
				case SWT.UP :
					return OS.ABS_UPDISABLED;
				case SWT.DOWN :
					return OS.ABS_DOWNDISABLED;
				case SWT.LEFT :
					return OS.ABS_LEFTDISABLED;
				case SWT.RIGHT :
					return OS.ABS_RIGHTDISABLED;
			}
		}

		switch (direction) {
			case SWT.UP :
				return OS.ABS_UPNORMAL;
			case SWT.DOWN :
				return OS.ABS_DOWNNORMAL;
			case SWT.LEFT :
				return OS.ABS_LEFTNORMAL;
			case SWT.RIGHT :
				return OS.ABS_RIGHTNORMAL;
		}
	...
	private static void handleDPIChange(Widget widget, int newZoom,
			float scalingFactor) {
		if (!(widget instanceof Button_Old button)) {
			return;
		}
		// Refresh the image
