...
public abstract class Control extends Widget implements Drawable {
	...
	static final boolean FORCE_RUN_UPDATE = Boolean
			.valueOf(System.getProperty("org.eclipse.swt.internal.control.forceRunUpdate"));
	...
	long accessibilityAttributeNames(long id, long sel) {
		...
		// If the SWT accessibility didn't give us anything get the default for the
		// view/cell.
		if (returnValue == 0)
			returnValue = super.accessibilityAttributeNames(id, sel);
	 ...
	 *         or overridden by the SWT Accessible. false if the Cocoa defaults for
	 *         the control should be used.
	 */
	...
	long accessibilityAttributeValue_forParameter(long id, long sel, long arg0, long arg1) {
		...
		if (handleIsAccessible(id) && accessible != null) {
			...
			returnValue = accessible.internal_accessibilityAttributeValue_forParameter(attribute, parameter,
					ACC.CHILDID_SELF);
		}
	 ...
	 * drag gesture occurs, by sending it one of the messages defined in the
	 * <code>DragDetectListener</code> interface.
	 *
	 * @param listener the listener which should be notified
	 *
	 * @exception IllegalArgumentException
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when
	 * gesture events are generated for the control, by sending it one of the
	 * messages defined in the <code>GestureListener</code> interface.
	 * <p>
	 * NOTE: If <code>setTouchEnabled(true)</code> has previously been invoked on
	 * the receiver then <code>setTouchEnabled(false)</code> must be invoked on it
	 * to specify that gesture events should be sent instead of touch events.
	 * </p>
	 * <p>
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when
	 * the platform-specific context menu trigger has occurred, by sending it one of
	 * the messages defined in the <code>MenuDetectListener</code> interface.
	 *
	 * @param listener the listener which should be notified
	 *
	 * @exception IllegalArgumentException
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when
	 * the mouse passes or hovers over controls, by sending it one of the messages
	 * defined in the <code>MouseTrackListener</code> interface.
	 *
	 * @param listener the listener which should be notified
	 *
	 * @exception IllegalArgumentException
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when
	 * the mouse wheel is scrolled, by sending it one of the messages defined in the
	...
	/**
	 * Adds the listener to the collection of listeners who will be notified when
	 * touch events occur, by sending it one of the messages defined in the
	 * <code>TouchListener</code> interface.
	 * <p>
	 * NOTE: You must also call <code>setTouchEnabled(true)</code> to specify that
	 * touch events should be sent, which will cause gesture events to not be sent.
	 * </p>
	 * <p>
	 * <b>Warning</b>: This API is currently only implemented on Windows and Cocoa.
	 * SWT doesn't send Gesture or Touch events on GTK.
	...
	void calculateVisibleRegion(NSView view, long visibleRgn, boolean clipChildren) {
		...
		OS.SetRect(rect, (short) bounds.x, (short) bounds.y, (short) (bounds.x + bounds.width),
				(short) (bounds.y + bounds.height));
	...
	NSAttributedString createString(String string, Font font, double[] foreground, int alignment, boolean wrap,
			boolean enabled, boolean mnemonics) {
		...
		NSAttributedString attribStr = ((NSAttributedString) new NSAttributedString().alloc()).initWithString(str,
				dict);
		str.release();
		dict.release();
		return attribStr;
	...
	void doCommandBySelector(long id, long sel, long selector) {
		if (hasKeyboardFocus(id)) {
			...
			if (nsEvent != null && nsEvent.type() == OS.NSKeyDown) {
				/*
				 * Feature in Cocoa. Pressing Alt+UpArrow invokes doCommandBySelector twice,
				 * with selectors moveBackward and moveToBeginningOfParagraph (Alt+DownArrow
				 * behaves similarly). In order to avoid sending multiple events for these keys,
				 * do not send a KeyDown if we already sent one during this keystroke. This rule
				 * does not apply if the command key is down, because we likely triggered the
				 * current key sequence via flagsChanged.
				 */
	...
	boolean dragDetect(int x, int y, boolean filter, boolean[] consume) {
		/**
		 * Feature in Cocoa. Mouse drag events do not account for hysteresis. As soon as
		 * the mouse drags a mouse dragged event is fired. Fix is to check for another
		 * mouse drag event that is at least 5 pixels away from the start of the drag.
		 */
		...
		/**
		 * To check for an actual drag we need to pull off mouse moved and mouse up
		 * events to detect if the user dragged outside of a 10 x 10 box centered on the
		 * mouse down location. We still want the view to see the events, so save them
		 * and re-post when done checking. If no motion happens for .3 seconds, bail.
		 */
		...
		while (eventType != OS.NSLeftMouseUp) {
			...
			if (eventType == OS.NSLeftMouseDragged) {
				...
				if ((Math.abs(viewLoc.x - dragX) > DEFAULT_DRAG_HYSTERESIS)
						|| (Math.abs(viewLoc.y - dragY) > DEFAULT_DRAG_HYSTERESIS)) {
					dragging = true;
					break;
				}
		...
		// Push back any events we took out of the queue so the control can receive
		// them.
	...
	void fillBackground(NSView view, NSGraphicsContext context, NSRect rect, int imgHeight, NSView gcView, int tx,
			int ty) {
		if (!drawsBackground())
			return;
	...
	public boolean forceFocus() {
		...
		shell.setSavedFocus(this);
		/*
		 * Feature in Cocoa. If the window is inactive when forceFocus is called
		 * bringToTop eventually calls makeKeyAndOrderFront. This activates the window
		 * immediately, but unlike other platforms, it also immediately fire
		 * notifications that the window was activated, as opposed to posting an event
		 * to be handled on the next pass of readAndDispatch().
		 *
		 * Shell#windowDidBecomeKey will call Decorations#restoreFocus, so the saved
		 * focus must be set before the window is activated or the wrong control will
		 * get focus.
		 */
	...
	boolean gestureEvent(long id, long eventPtr, int detail) {
		...
		// For cross-platform compatibility, touch events and gestures are mutually
		// exclusive.
	...
	public Monitor getMonitor() {
		...
		for (int i = 0; i < monitors.length; i++) {
			...
			int x = centerX < rect.x ? rect.x - centerX
					: centerX > rect.x + rect.width ? centerX - rect.x - rect.width : 0;
			int y = centerY < rect.y ? rect.y - centerY
					: centerY > rect.y + rect.height ? centerY - rect.y - rect.height : 0;
	...
	public long internal_new_GC(GCData data) {
		...
		if (graphicsContext == null) {
			NSWindow window = view.window();
			/*
			 * Force the device to be created before attempting to create a GC on a deferred
			 * NSWindow.
			 */
			...
			if (graphicsContext == null) {
				// create a bitmap based context, which will still work e.g. for text size
				// computations
	...
	boolean isShowing() {
		/*
		 * This is not complete. Need to check if the widget is obscurred by a parent or
		 * sibling.
		 */
		if (!isVisible())
			return false;
	...
	void keyDown(long id, long sel, long theEvent) {
		if (hasKeyboardFocus(id)) {
			Shell s = this.getShell();
			s.keyInputHappened = false;
			boolean textInput = OS.objc_msgSend(id, OS.sel_conformsToProtocol_,
					OS.objc_getProtocol("NSTextInput")) != 0;
			if (!textInput) {
				...
				// Control is some kind of text field, so the key event will be sent from
				// insertText: or doCommandBySelector:
	...
	long menuForEvent(long id, long sel, long theEvent) {
		...
		event.y = y;
		NSEvent nsEvent = new NSEvent(theEvent);
		event.detail = (nsEvent.type() == OS.NSLeftMouseDown || nsEvent.buttonNumber() > 0) ? SWT.MENU_MOUSE
				: SWT.MENU_KEYBOARD;
	...
	boolean mouseEvent(long id, long sel, long theEvent, int type) {
		...
		case OS.NSLeftMouseDown:
			if (nsEvent.clickCount() == 1 && (nsEvent.modifierFlags() & OS.NSControlKeyMask) == 0
					&& (state & DRAG_DETECT) != 0 && hooks(SWT.DragDetect)) {
	 ...
	 * @exception IllegalArgumentException
	 *                                     <ul>
	 *                                     <li>ERROR_NULL_ARGUMENT - if the gc is
	 *                                     null</li>
	 *                                     <li>ERROR_INVALID_ARGUMENT - if the gc
	...
	/**
	 * Requests that this control and all of its ancestors be repositioned by their
	 * layouts at the earliest opportunity. This should be invoked after modifying
	 ...
	 * fast-running and only marks the control for future participation in a
	 * deferred layout.
	 * <p>
...
//	checkWidget();
	...
	/**
	 * Removes the listener from the collection of listeners who will be notified
	 * when gesture events are generated for the control.
	 *
	 * @param listener the listener which should no longer be notified
	 *
	 * @exception IllegalArgumentException
	...
	void sendFocusEvent(int type) {
		...
		/*
		 * It is possible that the shell may be disposed at this point. If this happens
		 * don't send the activate and deactivate events.
		 */
		if (!shell.isDisposed()) {
			switch (type) {
			case SWT.FocusIn:
	 ...
	 * the argument. Note that on some platforms, a mouse button must currently be
	 * down for capture to be assigned.
	 *
	 * @param capture <code>true</code> to capture the mouse, and <code>false</code>
	 *                to release it
	 *
	 * @exception SWTException
	 *                         <ul>
	...
	void cacheDisplayInRect_toBitmapImageRep(long id, long sel, NSRect rect, long rep) {
		 ...
		 * to be sent. This leads to recursion if GC.copyArea() is called inside the
		 * PaintListener.
		 *
	...
	/**
	 * Changes the parent of the widget to be the one provided. Returns
	 * <code>true</code> if the parent is successfully changed.
	 *
	...
	/**
	 * Sets the shape of the control to the region specified by the argument. When
	 * the argument is null, the default shape of the control is restored.
	 *
	 ...
	 * not). Setting this to <code>false</code> causes the receiver to send gesture
	 * events instead. No exception is thrown if a touch-based input device is not
	 * detected (this can be determined with
	...
	public void setVisible(boolean visible) {
		...
		if (visible) {
			 ...
			 * widget in the show event. If this happens, just return.
			 */
			sendEvent(SWT.Show);
			if (isDisposed())
				return;
		...
		/*
		 * Feature in the Macintosh. If the receiver has focus, hiding the receiver
		 * causes no control to have focus. Also, the focus needs to be cleared from any
		 * TXNObject so that it stops blinking the caret. The fix is to assign focus to
		 * the first ancestor control that takes focus. If no control will take focus,
		 * clear the focus control.
		 */
		Control control = null;
		boolean fixFocus = false;
		if (!visible) {
	...
	boolean touchEvent(long id, long sel, long eventPtr) {
		...
		NSArray endedTouches = nsEvent.touchesMatchingPhase(OS.NSTouchPhaseEnded | OS.NSTouchPhaseCancelled, null)
				.allObjects();
		...
		NSArray activeTouches = nsEvent
				.touchesMatchingPhase(OS.NSTouchPhaseBegan | OS.NSTouchPhaseMoved | OS.NSTouchPhaseStationary, null)
				.allObjects();
		...
		if (activeTouches.count() != currentTouches.count()) {
			/**
			 * Bug in Cocoa. Under some situations we don't get the
			 * NSTouchPhaseEnded/Cancelled notification. Most commonly this happens if a
			 * 4-finger gesture occurs and the application switcher appears. Workaround is
			 * to generate a TOUCHSTATE_UP for the orphaned touch.
			 */
	...
	/**
	 * Performs a platform traversal action corresponding to a <code>KeyDown</code>
	 * event.
	 *
	 * <p>
	 ...
	 * and <code>SWT.TRAVERSE_PAGE_PREVIOUS</code>. If <code>traversal</code> is
	 * <code>SWT.TRAVERSE_NONE</code> then the Traverse event is created with
	 * standard values based on the KeyDown event. If <code>traversal</code> is one
	 * of the other traversal constants then the Traverse event is created with this
	 * detail, and its <code>doit</code> is taken from the KeyDown event.
	 * </p>
	 *
	 * @param traversal the type of traversal, or <code>SWT.TRAVERSE_NONE</code> to
	...
	boolean traverseGroup(boolean next) {
		...
		/*
		 * It is possible (but unlikely), that application code could have disposed the
		 * widget in focus in or out events. Ensure that a disposed widget is not
		 * accessed.
		 */
		if (index == length)
			return false;
