...
public class TableItem extends Item {
	...
	void doPaint(GC gc, int index) {
		...
		final Rectangle itemBounds = getBounds();
		...
		if (columnCount > 0) {
			for (int i = 0; i < columnCount; i++) {
				...
				Rectangle cellBounds = getBounds(i);
				...
				if (table.renderer.drawCell(this, i, detail, cellBounds, x, gc)) {
					drawFocusRect = true;
				}
			...
			drawFocusRect = table.renderer.drawCell(this, 0, detail, itemBounds, 0, gc);

			if ((table.getStyle() & SWT.CHECK) != 0) {
				final Rectangle fullBounds = getFullBounds();
	...
	private int prepareEventDetail(int index, Table table) {
		...
		if (table.mouseHoverElement == this) {
			detail |= SWT.HOT;
		}
